...
def choose_best_move(my_snake,evaluater):
    ...
    target_food = evaluater.get_best_food()
    head = my_snake.head

    # --- 評価ロジック (インデントを下げて常に実行) ---
    for move in safe_moves:
        ...
        if reachble_counts[move] >= MAX_DEPTH:
            ...
            score = (
                ...
                # tail_distancesはマイナス評価（尻尾に近いと危険）
                -1 * tail_distances[move] * TAIL_W + 
                (reachble_counts[move] - MAX_DEPTH) * 2 - 
                direction_counts[move] * DIRECTION_W - 
                food_next_counts[move] * FOOD_NEXT_W
            )
        else:
            score = reachble_counts[move] * 10
        
        if target_food:
            current_distance = abs(target_food['x'] - head['x']) + abs(target_food['y'] - head['y'])
            ...
            next_distance = abs(target_food['x'] - next_x) + abs(target_food['y'] - next_y)
...
#------------------------------------------------------------------------
    ...
    if len(super_safe_moves) > 0:
        best_move = max(super_safe_moves, key=lambda move: move_scores[move])
    else:
        # 超安全な手がない場合は、普通の安全な手からマシなものを選ぶ
